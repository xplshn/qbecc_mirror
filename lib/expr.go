...
func (c *ctx) convert(n cc.Node, dstType, srcType cc.Type, v any) (r any) {
	...
	case dstType.Kind() == cc.Double || dstType.Kind() == cc.LongDouble:
		...
		case c.isIntegerType(srcType):
			...
			switch srcSize {
			case 1, 2:
				w := c.temp("w ext%s %s", c.abiType(n, srcType), v)
		...
		case cc.ComplexDouble:
			...
			case *escapedVar:
				...
				case cc.Complex128Value:
					r = c.temp("%s add %%.bp., %v\n", c.wordTag, x.offset)
					c.w("\tstored %v, %v\n", float64Value(real(y)), r)
					im := c.temp("%s add %s, 8\n", c.wordTag, r)
					c.w("\tstored %v, %v\n", float64Value(imag(y)), im)
					return r
				default:
					panic(todo("%v: %s %s %T %v", n.Position(), mode, cc.NodeSource(n), y, t.Kind()))
				}
			default:
				panic(todo("%v: %s %s %T %v %T", n.Position(), mode, cc.NodeSource(n), v, t.Kind(), x))
			}
		default:
			panic(todo("%v: %s %s %T %v", n.Position(), mode, cc.NodeSource(n), v, t.Kind()))
		}
	default:
		panic(todo("%v: %s %s %T", n.Position(), mode, cc.NodeSource(n), v))
	}
...
func (c *ctx) vaArg(n *cc.PostfixExpression, mode mode, t cc.Type) (r any) {
	...
	case rvalue, void:
		...
		case d != nil && d.IsParam():
			return c.temp("%s vaarg %s\n", c.baseType(n, t), info.(*localVar).name)
		default:
...
func (c *ctx) postfixExpressionCall(n *cc.PostfixExpression, mode mode, t cc.Type) (r any) {
	if x, ok := unparen(n.PostfixExpression).(*cc.PrimaryExpression); ok {
		...
		case cc.PrimaryExpressionGeneric:
			nm := cc.NodeSource(x.GenericSelection.Associated().AssignmentExpression)
			switch nm {
			case "__isfinitef":
			...
			switch {
			case strings.HasPrefix(nm, "__atomic_"):
				c.needAtomics = true
			}
		}
	}
	...
	for iArg, l := 0, n.ArgumentExpressionList; l != nil; iArg, l = iArg+1, l.ArgumentExpressionList {
		...
		default:
			switch {
			case c.isComplexType(et):
				expr = c.expr(e, aggRvalue, et)
			case c.isAggType(et):
...
func (c *ctx) unaryExpressionCpl(n *cc.UnaryExpression, mode mode, t cc.Type) (r any) {
	...
	case constRvalue:
		switch x := c.value(n, constRvalue, t, n.Value(), false).(type) {
		case int64Value:
			return ^x
		case uint64Value:
			return ^x
		default:
			panic(todo("%v: %v %s %s<-%s %T", n.Position(), mode, cc.NodeSource(n), t, n.Type(), x))
		}
	default:
		// "complex-6.c": {},
		panic(todo("%v: %v %s %s<-%s", n.Position(), mode, cc.NodeSource(n), t, n.Type()))
...
func (c *ctx) arithmeticOpRvalue(n, lhs, rhs cc.ExpressionNode, mode mode, t cc.Type, op string, ct cc.Type, lmul, rmul, div int64) (r any) {
	// trc("%s: lhs=%s rhs=%s mode=%v t=%v op=%s ct=%v", n.Position(), cc.NodeSource(lhs), cc.NodeSource(rhs), mode, t, op, ct)
...
func (c *ctx) arithmeticOpAggRvalueComplexFloat(n, lhs, rhs cc.ExpressionNode, mode mode, t cc.Type, op string, lt, rt, ct cc.Type, lmul, rmul, div int64) (r any) {
	if lmul != 1 || rmul != 1 || div != 1 {
		panic(todo("%v: %v %s %s %s", lhs.Position(), mode, cc.NodeSource(lhs), op, cc.NodeSource(rhs)))
	}
	...
	case *cc.UnknownValue:
		...
		case *cc.UnknownValue:
			switch lt.Kind() {
			case cc.ComplexFloat:
				p := c.expr(rhs, aggRvalue, lt)
				a = c64{re: ptr{p}}
				a.im = ptr{c.temp("%s add %s, 8\n", c.wordTag, p)}
			default:
				panic(todo("%v: %v %s %s %s lt=%v", lhs.Position(), mode, cc.NodeSource(lhs), op, cc.NodeSource(rhs), lt))
			}
			switch rv := rhs.Value().(type) {
			case *cc.UnknownValue:
				...
				case cc.ComplexFloat:
					...
					b.im = ptr{c.temp("%s add %s, 8\n", c.wordTag, p)}
				default:
					panic(todo("%v: %v %s %s %s rt=%v", lhs.Position(), mode, cc.NodeSource(lhs), op, cc.NodeSource(rhs), rt))
				}
			default:
				_ = b
				panic(todo("%v: %v %s op=%s %s %T %T", lhs.Position(), mode, cc.NodeSource(lhs), op, cc.NodeSource(rhs), lv, rv))
	...
	default:
		...
		case *escapedVar:
			...
			case cc.ComplexFloat:
				r = c.temp("%s add %%.bp., %v\n", c.wordTag, x.offset)
				im := c.temp("%s add %s, 4\n", c.wordTag, r)
				ar, ai := c.complexFloat(n, a)
				br, bi := c.complexFloat(n, b)
				switch op {
				case "add", "sub":
					// (ar + ai*i) + (br + bi*i) = (ar+br) + (ai+bi)i
					x := c.temp("s %s %s, %s\n", op, ar, br)
...
func (c *ctx) complexFloat(n cc.Node, v c64) (re, im any) {
	return c.complexFloatPart(n, v.re), c.complexFloatPart(n, v.im)
}

func (c *ctx) complexFloatPart(n cc.Node, v any) any {
	switch x := v.(type) {
	case ptr:
		return c.load(n, x.any, c.ast.Float)
	default:
		return x
	}
}
...
func (c *ctx) arithmeticOp(n, lhs, rhs cc.ExpressionNode, mode mode, t cc.Type, op string) (r any) {
	...
	switch mode {
	case void:
		c.arithmeticOpRvalue(n, lhs, rhs, rvalue, n.Type(), op, ct, lmul, rmul, div)
	case rvalue:
	...
	case aggRvalue:
		switch ct.Kind() {
		case cc.ComplexFloat:
			return c.arithmeticOpAggRvalueComplexFloat(n, lhs, rhs, mode, t, op, lt, rt, ct, lmul, rmul, div)
...
func (c *ctx) toBaseType(n cc.Node, v any, t cc.Type) (r any) {
	if c.isIntegerType(t) && t.Size() < 4 {
		return c.convert(n, c.ast.Int, t, v)
	}

	return v
}

// returns w 0 or w 1 only.
func (c *ctx) bool(n cc.ExpressionNode) (r any) {
	switch {
	case c.isZero(n):
		c.expr(n, rvalue, n.Type())
		return c.temp("w copy 0\n")
	case c.isNonzero(n):
		c.expr(n, rvalue, n.Type())
		return c.temp("w copy 1\n")
...
func (c *ctx) logicalOrExpressionLOr(n *cc.LogicalOrExpression, mode mode, t cc.Type) (r any) {
	...
	case void:
		...
		case c.isZero(n.LogicalOrExpression):
			...
			return c.bool(n.LogicalAndExpression)
		case c.isNonzero(n.LogicalOrExpression):
			return c.bool(n.LogicalOrExpression)
		default:
			a := c.label()
			z := c.label()
			//	%r = bool(orExpr)
			...
			//	%r = bool(andExpr)
			// @z
			r = c.bool(n.LogicalOrExpression)
			c.w("\tjnz %s, %s, %s\n", r, z, a)
			c.w("%s\n", a)
			e2 := c.bool(n.LogicalAndExpression)
			c.w("\t%s =w copy %s\n", r, e2)
			c.w("%s\n", z)
			return r
		}
...
func (c *ctx) conditionalExpressionCond(n *cc.ConditionalExpression, mode mode, t cc.Type) (r any) {
	...
	x := c.label()
	z := c.label()
	e := c.bool(n.LogicalOrExpression)
	rt := n.LogicalOrExpression.Type()
	if n.ExpressionList != nil {
		rt = n.ExpressionList.Type()
	}
	switch mode {
	case lvalue:
		defer func() { r = c.convert(n, t, n.Type(), r) }()
		...
		switch {
		case c.isZero(n.LogicalOrExpression):
			return c.expr(n.ConditionalExpression, mode, rt)
		case c.isNonzero(n.LogicalOrExpression):
			switch {
			case n.ExpressionList == nil:
				return c.convert(n, t, c.ast.Int, e)
			default:
				return c.expr(n.ExpressionList, mode, rt)
			}
		default:
			c.w("\tjnz %s, %s, %s\n", e, a, b)
			c.w("%s\n", a)
			switch {
			case n.ExpressionList == nil:
				r = c.convert(n, t, c.ast.Int, e)
			default:
				r = c.expr(n.ExpressionList, mode, rt)
			}
			c.w("%s\n", x)
			c.w("\tjmp %s\n", z)
			c.w("%s\n", b)
			r = c.expr(n.ConditionalExpression, mode, rt)
			c.w("%s\n", z)
			return r
		}
	case rvalue:
		defer func() { r = c.convert(n, t, rt, r) }()
		...
		default:
			...
			switch {
			case n.ExpressionList == nil:
				el = c.convert(n, t, c.ast.Int, e)
			default:
				el = c.expr(n.ExpressionList, mode, rt)
			}
			r = c.temp("%s copy %s\n", c.baseType(n, rt), el)
			c.w("%s\n", x)
			c.w("\tjmp %s\n", z)
			c.w("%s\n", b)
			ce := c.expr(n.ConditionalExpression, mode, rt)
			c.w("\t%s =%s copy %s\n", r, c.baseType(n, rt), ce)
			c.w("%s\n", z)
			return r
		}
	case void:
		...
		default:
			...
			c.w("%s\n", a)
			if n.ExpressionList != nil {
				c.expr(n.ExpressionList, mode, rt)
			}
			c.w("%s\n", x)
			c.w("\tjmp %s\n", z)
			c.w("%s\n", b)
			c.expr(n.ConditionalExpression, mode, rt)
			c.w("%s\n", z)
			return r
		}
	case aggRvalue:
